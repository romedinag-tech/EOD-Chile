...
// ── Mapa Leaflet — helpers del sitio de referencia ───────────────────────────
...
function render(){
  ...
  document.getElementById("res-lead").innerHTML=
    ...
    (d.zonas?d.zonas.length:0)+' zonas con centroide georeferenciado.';
...
function renderResumen(d){
  const k=d.kpis;
  const viajes=fmtM(k.total_viajes);
  const tiempo=k.tiempo_medio_min!=null?fmt(k.tiempo_medio_min,0)+" min":"s/d";
  const pub   =k.pct_publico!=null?fmt(k.pct_publico,1)+"%":"s/d";
  const priv  =k.pct_privado!=null?fmt(k.pct_privado,1)+"%":"s/d";
  const nm    =k.pct_no_motorizado!=null?fmt(k.pct_no_motorizado,1)+"%":"s/d";
  const topProp=d.proposito&&d.proposito.length?[...d.proposito].sort((a,b)=>b.pct-a.pct)[0]:null;

  let html=`
    <div class="kpi"><div class="v">${viajes}</div><div class="l">Viajes en día laboral</div><div class="s">viajes expandidos</div></div>
    <div class="kpi"><div class="v">${tiempo}</div><div class="l">Tiempo medio de viaje</div><div class="s">minutos por viaje</div></div>
    <div class="kpi"><div class="v" style="color:${NAVY}">${pub}</div><div class="l">Transporte público</div><div class="s">% del total</div></div>
    <div class="kpi"><div class="v" style="color:${RED}">${priv}</div><div class="l">Transporte privado</div><div class="s">automóvil y moto</div></div>
    <div class="kpi"><div class="v" style="color:${GREEN}">${nm}</div><div class="l">No motorizado</div><div class="s">caminata y bicicleta</div></div>`;
  if(k.pct_combinado>0.5)
    html+=`<div class="kpi"><div class="v" style="color:${OR}">${fmt(k.pct_combinado,1)}%</div><div class="l">Combinado</div><div class="s">multi-modal</div></div>`;
  if(topProp)
    html+=`<div class="kpi"><div class="v or">${fmt(topProp.pct,1)}%</div><div class="l">Propósito principal</div><div class="s">${topProp.prop}</div></div>`;
...
function drawCircles(d,view){
  const zones=d.zonas;
  if(!zones||!zones.length){odInfo.update("Sin datos de zonas para esta ciudad.");return;}
  ...
  // leyenda
  const szs=[["8px","8px","0.4"],["14px","14px","0.65"],["20px","20px","0.85"]];
...
function drawODLines(d){
  ...
  if(!flows||!flows.length){odInfo.update("Sin datos de flujos OD para esta ciudad.");return;}
